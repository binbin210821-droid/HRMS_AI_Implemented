...
| MongoDB (local replica set / production Atlas target) | Lưu dữ liệu nghiệp vụ, audit, context AI và Change Streams; `.env.production` trên EC2 có `MONGO_URI` trỏ tới Atlas theo cấu hình cung cấp. Audit source hiện chưa xác minh kết nối runtime, cluster/user/network, Change Streams hoặc index trên Atlas. Cluster/user/network values không lưu trong repo. |
...
- `.env.production` đã được xác nhận có cấu hình cho MongoDB Atlas, S3 bucket AWS và Redis rate limiting; riêng kết nối/runtime MongoDB Atlas chưa được kiểm thử trong audit này. Chỉ ghi tên biến/trạng thái, không ghi URI, credential hoặc tên tài nguyên nhạy cảm.
...
- Production EC2, `.env.production`, cấu hình mục tiêu MongoDB Atlas, S3 bucket, Redis configuration, CI/CD và Cloudflare AI đã được xác minh theo thông tin runtime do chủ dự án cung cấp. Kết nối thực tế và các thiết lập Atlas chi tiết vẫn cần bằng chứng runtime riêng. Domain/HTTPS vẫn chưa cấu hình; HTTPS/Secure cookie, S3/Redis end-to-end và các điều kiện staging cũng vẫn cần bằng chứng runtime riêng.
...
| E-02 | Mục 2, EC2 deploy host; Mục 4 NFR-DEP-01 | Terminal EC2 gồm `hostname`, `uname -a`, phiên bản Docker, `docker compose ps` thành công và `curl http://127.0.0.1/api/v1/health` | SSH target là môi trường Ubuntu/AWS EC2, container production đang chạy và health check chuẩn hoạt động | Cần lưu ảnh bản `docker compose ps` thành công; ảnh lỗi đường dẫn không đủ làm bằng chứng |
| E-03 | Mục 2 MongoDB; Mục 6 database runtime | Terminal hoặc editor hiển thị **tên biến** trong `.env.production` như `MONGO_URI`, không hiển thị credential; kèm kết quả health check nếu có | Production container có connection target MongoDB; cần ping/runtime smoke test riêng để chứng minh backend truy cập đúng Atlas | Đã xác minh biến cấu hình; chưa đủ bằng chứng runtime Atlas |
...
| E-07 | Mục 2 MongoDB; Mục 6 database | MongoDB Atlas hiển thị cluster đang hoạt động, Network Access có IP/VPC của môi trường ứng dụng và Database User; che username nhạy cảm nếu cần | Xác nhận MongoDB Atlas cluster và quyền truy cập mạng/database đã được cấu hình, sau đó cần ping/runtime smoke test để chứng minh backend kết nối được | Mới xác nhận `.env.production` có connection target; chưa xác minh Atlas console, network hoặc runtime connection |
