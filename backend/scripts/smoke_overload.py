...
def main(args: argparse.Namespace) -> None:
    ...
    status, scan = request_json(args.base_url, "/api/overload/scan", leadership_token, "POST")
    ...
    status, leadership_logs = request_json(args.base_url, "/api/overload", leadership_token)
    ...
    assert all(
        candidate["employee_code"].startswith("KD-") for candidate in log_a["suggested_candidates"]
    )

    status, alerts = request_json(args.base_url, "/api/alerts?alert_type=all", leadership_token)
    ...
    status, manager_logs = request_json(args.base_url, "/api/overload", manager_token)
    assert status == 200, manager_logs
    assert manager_logs and all(
        log["department_id"] == log_a["department_id"] for log in manager_logs
    )
    assert all(log["employee_code"].startswith("KD-") for log in manager_logs)
